...
def convert_paddle_checkpoint_to_pytorch(paddle_checkpoint_path, pytorch_dump_path):
    ...
    from paddlenlp.transformers import ErnieForPretraining as PErnieForPreTraining, ErnieTokenizer as PErnieTokenizer, \
        ErnieCtmModel as PErnieCtmModel, ErnieCtmWordtagModel as PErnieCtmForWordtag, ErnieCtmTokenizer as PErnieCtmTokenizer
    ...
    model_type = "ernie"
    if "ernie-ctm" in paddle_checkpoint_path:
        model_type = "ernie-ctm"
    elif "wordtag" in paddle_checkpoint_path:
        model_type = "wordtag"

    src_model_cls_map = {
        "ernie": PErnieForPreTraining,
        "wordtag": PErnieCtmForWordtag,
        "ernie-ctm": PErnieCtmModel,
    }
    src_tok_cls_map = {
        "ernie": PErnieTokenizer,
        "wordtag": PErnieCtmTokenizer,
        "ernie-ctm": PErnieCtmTokenizer,
    }
    tgt_model_cls_map = {
    ...
    src_cls = src_model_cls_map[model_type]
    tgt_cls = tgt_model_cls_map[model_type]
    ...
    config = src_model.config
    if "ernie" in config:
        config = config["ernie"].config
    if "ernie_ctm" in config:
        config = config["ernie_ctm"].config
    config = {k: v for k, v in config.items() if k != "self"}
    config = ErnieCtmConfig(**config)

    tokenizer_cls = src_tok_cls_map[model_type]
    tokenizer = tokenizer_cls.from_pretrained(paddle_checkpoint_path)
