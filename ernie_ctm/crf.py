...
def log_sum_exp(vec, dim=0):
    ...
    max_num = torch.max(vec, dim)[0]
    max_exp = max_num.unsqueeze(-1)
    return max_num + torch.log(torch.sum(torch.exp(vec - max_exp), dim))
...
class LinearChainCrf(nn.Module):
    ...
    def __init__(self, num_labels, crf_lr=0.1, with_start_stop_tag=True):
        ...
        self.transitions = nn.Parameter(torch.full((self.num_tags, self.num_tags), crf_lr, dtype=torch.float32))
    ...
    def _initialize_alpha(self, batch_size):
        ...
        if self._initial_alpha is None or batch_size > self._initial_alpha.shape[0]:
            ...
            alpha_start = torch.full(
                (batch_size, 1), dtype=torch.float32, fill_value=0.)
            self._initial_alpha = torch.concat(
    ...
    def forward(self, inputs, lengths):
        ...
        inputs_t_exp = inputs.permute([1, 0, 2]).unsqueeze(-1)
        ...
        # Get the valid alpha
        all_alpha = torch.stack(all_alpha).permute([1, 0, 2])
        batch_index = self._get_batch_index(batch_size)
        last_index = lengths - 1
        alpha = all_alpha[batch_index, last_index]
    ...
    def _point_score(self, inputs, labels, lengths):
        ...
        offsets = torch.unsqueeze(self._get_batch_index(batch_size) * seq_len * n_labels, 1).expand(-1, seq_len) + \
                torch.unsqueeze(self._get_seq_index(seq_len) * n_labels, 0).expand(batch_size, -1)
        flattened_tag_indices = torch.reshape(offsets + labels, [-1])

        scores = torch.gather(flattened_inputs, 0, flattened_tag_indices).reshape(
            [batch_size, seq_len])

        mask = sequence_mask(self._get_batch_seq_index(batch_size, seq_len), lengths)
        mask = mask[:, :seq_len]

        mask_scores = scores * mask
        score = torch.sum(mask_scores, 1)
        return score
    ...
    def _trans_score(self, labels, lengths):
        ...
        if self.with_start_stop_tag:
            ...
            labels_ext = torch.concat(
                [start_tensor, labels, stop_tensor], axis=1)
            mask = sequence_mask(self._get_batch_seq_index(batch_size, seq_len), lengths + 1)
            pad_stop = torch.full(
        ...
        flattened_transition_params = torch.flatten(self.transitions)
        scores = torch.gather(
    ...
    def _get_start_stop_tensor(self, batch_size):
        if self._start_tensor is None or self._stop_tensor is None or batch_size != self._start_tensor.shape[0]:
            self._start_tensor = torch.full(
                (batch_size, 1), dtype=torch.int64, fill_value=self.start_idx)
            self._stop_tensor = torch.full(
                (batch_size, 1), dtype=torch.int64, fill_value=self.stop_idx)
    ...
    def _get_batch_index(self, batch_size):
        if self._batch_index is None or batch_size != self._batch_index.shape[0]:
            self._batch_index = torch.arange(end=batch_size, dtype=torch.int64)
        return self._batch_index

    def _get_seq_index(self, length):
        if self._seq_index is None or length > self._seq_index.shape[0]:
            self._seq_index = torch.arange(end=length, dtype=torch.int64)
    ...
    def _get_batch_seq_index(self, batch_size, length):
        if self._batch_seq_index is None or length + 2 > self._batch_seq_index.shape[
            ...
            self._batch_seq_index = torch.cumsum(
                torch.ones(batch_size, length + 2, dtype=torch.int64), axis=1) - 1
        if self.with_start_stop_tag:
...
class ViterbiDecoder(nn.Module):
    ...
    def forward(self, inputs, lengths):
        ...
        if self.with_start_stop_tag:
            ...
            alpha = torch.zeros((batch_size, self.num_tags), dtype=torch.float32)
        for i, logit in enumerate(inputs_t[:max_seq_len]):
            ...
            alpha_max = alpha_trn_sum.max(1)[0]
            ...
            mask = left_length > 0
            alpha = mask * alpha_nxt + (1 - mask.long()) * alpha
        ...
        historys = torch.flip(historys, [0])
        for hist in historys:
            # hist: batch_size, n_labels
            left_length = left_length + 1
            gather_idx = batch_offset + last_ids
            tag_mask = left_length > 0
            last_ids_update = torch.gather(hist.flatten(),
                                           0,
                                            gather_idx) * tag_mask
            zero_len_mask = left_length == 0
            last_ids_update = last_ids_update * (1 - zero_len_mask.long()) + last_ids * zero_len_mask
            batch_path.append(last_ids_update)
            tag_mask = left_length >= 0
            last_ids = last_ids_update + last_ids * (1 - tag_mask.long())
        batch_path = torch.flip(torch.stack(batch_path, 1), [1])
